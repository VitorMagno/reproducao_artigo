...
def criar_modelo_cnn(input_shape=(224, 224, 3), task='drowsiness'):
    ...
    x = layers.Dropout(rate=0.35, name='dropout2')(x)
    ...
    x = layers.Dropout(rate=0.45, name='dropout3')(x)
    ...
    # Camada 4: Dropout
    x = layers.Dropout(rate=0.5, name='dropout_fc1')(x)
    
    # Camada 5: Dense (segunda camada totalmente conectada)
    x = layers.Dense(units=256, activation='relu', name='fc2')(x)
    ...
    if task == 'drowsiness':
        ...
        outputs = layers.Dense(units=1, activation='sigmoid', name='output_binary')(x)
        loss = 'binary_crossentropy'
        metrics = ['accuracy']
    elif task == 'distraction':
        # Classificação multi-classe: 10 classes de distração
        outputs = layers.Dense(units=10, activation='softmax', name='output_multiclass')(x)
    ...
    # Criar modelo
    model = Model(inputs=inputs, outputs=outputs, name=f'CNN_{task}_detection')
